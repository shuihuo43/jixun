...
/// <summary>恶灵：出生扣玩家 0.5% 血，碰敌自毁</summary>
public class WraithEntity_Evil : ElementEntity
{
    [Header("消耗")]
    [SerializeField] private float hpCost = 1f;
    ...
    public override void EntityBorn(Vector2 pos, Vector2 dir, GameObject root, Vector2? scale = null, bool flipY = false, System.Action onDestroy = null, GameObject ownerObj = null)
    {
        ...
        p?.PlayerResource?.ChangeHealth(-hpCost);
    }
    ...
    {
        base.Update();
        if (target == null || !IsAlive(target)) PickRandomTarget();
        switchTimer -= Time.deltaTime;
        if (switchTimer <= 0f) PickRandomTarget();
        if (target != null)
        {
